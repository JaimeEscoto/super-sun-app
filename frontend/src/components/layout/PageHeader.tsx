    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/90 px-6 py-6 shadow-xl">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-3">
          {badge && (
            <span className="inline-flex items-center gap-2 rounded-full border border-primary/40 bg-primary/20 px-3 py-1 text-xs font-semibold uppercase tracking-[0.35em] text-primary">
              {badge}
            </span>
          )}
        ...
        {actions && <div className="flex flex-wrap items-center gap-3 text-slate-200">{actions}</div>}
      </div>
    </div>
  );
};
